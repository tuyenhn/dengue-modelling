...
-   Now we fit 2 Poisson regression models, one model will have untransformed non-lagged covariables, the other model will have accumulated and lagged covariables. The forecast horizon will be 4 weeks (i.e. week 1-4 in 2019)
...
forecast1 <- poisson_fit %>%
  predict(newdata = test_weekly_weather_df %>% head(4)) %>%
  as.vector() %>%
  exp()
actual1 <- test_weekly_weather_df %>%
  head(4) %>%
  pull(n)
rmse1 <- sqrt(mean((forecast1 - actual1)^2))
mae1 <- mean(abs(forecast1 - actual1))

poisson_accum_lagged_fit <- glm(
  n ~ date_admitted + t2m + precip + rh,
  family = poisson(), data = train_weekly_accum_df
)
forecast2 <- poisson_accum_lagged_fit %>%
  predict(newdata = test_weekly_accum_df) %>%
  as.vector() %>%
  exp()
actual2 <- test_weekly_accum_df %>% pull(n)
rmse2 <- sqrt(mean((forecast2 - actual2)^2))
mae2 <- mean(abs(forecast2 - actual2))

knitr::kable(
  tibble(
    model = c("Poisson + raw covariables", "Poisson + accumulated, lagged covariables"),
    rmse = c(rmse1, rmse2),
    mae = c(mae1, mae2)
  )
)
...
-   At every lag $k$ for covariables, we perform a summation weighted by a kernel. The shape of this kernel will be defined by $\beta$.
