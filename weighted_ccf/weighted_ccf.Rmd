...
hyper_grid <- function(covarDf, varcol) {
  # param grid
  lags <- 0:52
  alphas <- 1:52
...
alpha_precip_perfs <- hyper_grid(covarDf = hcmc_precip_df, varcol = "precip")
alpha_temp_perfs <- hyper_grid(covarDf = hcmc_temp_df, varcol = "t2m")
alpha_rh_perfs <- hyper_grid(covarDf = hcmc_rh_df, varcol = "rh")
```
