...
incidence_full_renamed %>%
  replace_na(list(n = 0)) %>%
  mutate(date_admitted = as.Date(date_admitted), hospital = as.factor(hospital)) %>%
  group_by(hospital) %>%
  mutate(total = sum(n)) %>%
  ungroup() %>%
  mutate(hospital = fct_reorder(hospital, desc(total))) %>%
  ggplot() +
  geom_raster(aes(x = date_admitted, y = hospital, fill = n)) +
...
incidence_full_renamed %>%
  replace_na(list(n = 0)) %>%
  mutate(exist = ifelse(n > 0, 1, 0), date_admitted = as.Date(date_admitted)) %>%
  group_by(date_admitted) %>%
  summarise(hit = sum(exist) / n()) %>%
  ggplot() +
  geom_line(aes(x = date_admitted, y = hit)) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y")
```
