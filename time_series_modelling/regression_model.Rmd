...
tsglm <- recur_miso(
  train_weekly_weather_df, test_weekly_weather_df, seq(1, 49, 3),
  xreg_names = c("t2m"), link = "log", distr = "poisson"
) %>%
  mutate(
    date_admitted = yearweek(paste0("2019 W", actualweek)),
    .before = everything()
...
ggplot(test_weekly_df) +
  geom_line(aes(x = date_admitted, y = n)) +
  forecast_layer(tsglm %>% rename(n = dist, .mean = preds), funnel = FALSE) +
  forecast_layer(base_glm %>% rename(.mean = preds, lowers = lower, uppers = upper), funnel = FALSE)
```
...
glm(
  as.formula(paste0("n", stepwise_forward$final)),
  family = poisson,
  data = train_weekly_weather_df %>%
    as_tibble() %>%
    select(-date_admitted)
) %>%
  predict(newdata = test_weekly_weather_df %>% head(4), type = "response")
```
