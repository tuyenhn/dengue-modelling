...
```{r}
forecast_p <- ggplot(incidence_weekly_df, aes(x = date_admitted, y = n)) +
  forecast_layer(sarima_fits) +
  geom_line() +
...
```{r}
forecast_perf(
  (sarima_fits$arima_211110 %>% forecast(h = 4))[[1]] %>%
    as_tibble(.name_repair = "check_unique") %>% pull(.mean),
...
forecast_perf(
  (sarima_fits$arima_411110 %>% forecast(h = 4))[[1]] %>%
    as_tibble(.name_repair = "check_unique") %>% pull(.mean),
  test_weekly_df %>% pull(n),
  "SARIMA(4,1,1)(1,1,0)"
)
...
forecast_perf(
  (sarima_fits$auto_arima %>% forecast(h = 4))[[1]] %>%
    as_tibble(.name_repair = "check_unique") %>% pull(.mean),
  test_weekly_df %>% pull(n),
  "SARIMA(2,0,1)(1,1,0)"
)
...
```{r}
sarimax_fits %>%
  forecast(h = test_weekly_weather_df) %>%
  append_row(-1) %>%
  mutate(
    n = case_when(is.na(n) ~ mrdp_ci, .default = n),
    .mean = case_when(is.na(.mean) ~ mrdp$n, .default = .mean)
...
forecast_p <- ggplot(incidence_weekly_df, aes(x = date_admitted, y = n)) +
  forecast_layer(sarimax_fits, h = test_weekly_weather_df) +
  geom_line() +
  scale_x_yearweek("Week", limits = c(as.Date("2018-01-01"), as.Date("2019-01-21"))) +
  scale_y_continuous("Incidence") +
  ggtitle("4 week forecast - SARIMA")
