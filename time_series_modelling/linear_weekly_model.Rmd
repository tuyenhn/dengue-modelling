...
```{r}
boxcox_mle <- unclass(MASS::boxcox(lm(train_weekly_df$n ~ 1))) %>% as_tibble()
...
```{r}
fit <- sarima_fits %>% select(arima_211110)
...
sarimax_fits <- train_weekly_weather_df %>% model(
  # raw covariables
  arima_211110_t2m = ARIMA(n ~ xreg(t2m) + pdq(2, 1, 1) + PDQ(1, 1, 0)),
  arima_211110_precip = ARIMA(n ~ xreg(precip) + pdq(2, 1, 1) + PDQ(1, 1, 0)),

  # unit root issues so this model will be auto_arima
  auto_arima_weather = ARIMA(n ~ xreg(precip, t2m) + pdq(2, 0, 1) + PDQ(1, 1, 0)),

  # scaled covariables
  arima_211110_scaled_t2m = ARIMA(n ~ xreg(scaled_t2m) + pdq(2, 1, 1) + PDQ(1, 1, 0)),
  arima_211110_scaled_precip = ARIMA(n ~ xreg(scaled_precip) + pdq(2, 1, 1) + PDQ(1, 1, 0)),
  arima_211110_scaled_weather = ARIMA(n ~ xreg(scaled_precip, scaled_t2m) + pdq(2, 1, 1) + PDQ(1, 1, 0)),

  # mix of both
  arima_211110_mixed_st2m_rprecip = ARIMA(n ~ xreg(scaled_t2m, precip) + pdq(2, 1, 1) + PDQ(1, 1, 0)),
)
...
sarimax_fit <- sarimax_fits %>% select(arima_211110_scaled_weather)
...
forecast_p <- forecasted_sarimax %>%
  ...
  scale_x_yearweek("Date", date_breaks = "1 week", date_labels = "W%V-%Y") +
  scale_y_continuous("Incidence (log10-transformed)") + scale_alpha_continuous("Confidence interval")
...
```{r}
fit <- sarimax_fits %>% select(arima_211110_precip)
...
for (i in 1:4) {
  ...
  forecast_1w_weather_df %<>% tail(-1) %>% bind_rows(forecasted_1w)

  fit %<>% refit(forecast_1w_weather_df, reestimate = FALSE)
}
