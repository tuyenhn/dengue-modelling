...
naive_forecasts <- naive_fit %>% forecast(h = 4)

for (i in 1:13) {
  naive_forecasts %<>% bind_rows(
    naive_fit %>%
      refit(new_data = train_weekly_df %>% bind_rows(test_weekly_df %>% head(i * 4))) %>%
      forecast(h = 4)
  )
}
...
ggplot() +
  (naive_forecasts %>%
    ...
    as_fable(c(".mean"), "n") %>%
    autolayer()) +
  geom_line(data = incidence_weekly_df, aes(x = date_admitted, y = n)) +
  scale_x_yearweek("Week", limits = c(as.Date("2018-01-01"), as.Date("2019-12-31")))
```
...
ggsave("forecast_smean_4w.svg", width = 10, height = 7)
