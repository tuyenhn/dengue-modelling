...
ma_fit <- forecast_chunks(
  train_weekly_df, test_weekly_df, seq(0, 49, 3),
  ARIMA, n ~ pdq(0, 0, 6) + PDQ(0, 0, 0)
)

forecast_plot_default(
  ma_fit %>% mutate(across(c(.mean, uppers, lowers), exp)),
  test_weekly_df %>% mutate(n = exp(n)),
...
# )
# stopCluster(cl)
sarima_fit <- read_rds("sarima_fit.rds")
