...
sarimax_t2m_coefs <- sarimax_t2m_fit %>% coef() %>% select(c(term, estimate)) %>% deframe() %>% as.list()
sarimax_t2m_coefs

sarimax_precip_fit <- sarimax_models[[2]]
report(sarimax_precip_fit)
sarimax_precip_coefs <- sarimax_precip_fit %>% coef() %>% head(-1) %>% select(c(term, estimate)) %>% deframe() %>% as.list()
sarimax_precip_coefs

sarimax_rh_fit <- sarimax_models[[3]]
report(sarimax_rh_fit)
sarimax_rh_coefs <- sarimax_rh_fit %>% coef() %>% head(-1) %>% select(c(term, estimate)) %>% deframe() %>% as.list()
sarimax_rh_coefs
```
...
sarimax_t2m_chunks_fit <- forecast_chunks(
  train_weekly_weather_df, test_weekly_weather_df, seq(0, 49, 3),
  ARIMA, n ~ xreg(t2m, fixed = sarimax_t2m_coefs[7]) + pdq(0,1,5, fixed = sarimax_t2m_coefs[1:5]) + PDQ(1, 0, 0, fixed = sarimax_t2m_coefs[6])
)

sarimax_precip_chunks_fit <- forecast_chunks(
  train_weekly_weather_df, test_weekly_weather_df, seq(0, 49, 3),
  ARIMA, n ~ xreg(precip, fixed = sarimax_precip_coefs[4]) + pdq(1,0,1, fixed = sarimax_precip_coefs[1:2]) + PDQ(1, 1, 0, fixed = sarimax_precip_coefs[3])
)
...
forecast_plot_default(
  ...
  test_weekly_df %>% mutate(n = exp(n)),
  "4 week forecast - SARIMA(0,1,5)(1,0,0) + mean 2m temperature"
)
...
```{r}
t2m_pre_perf <- sarimax_t2m_chunks_fit %>%
  select(date_admitted, .mean) %>%
  left_join(test_weekly_df, by = "date_admitted")

forecast_perf(
  exp(t2m_pre_perf$.mean),
  exp(t2m_pre_perf$n), "SARIMA(0,1,5)(1,0,0) + t2m"
)

precip_pre_perf <- sarimax_precip_chunks_fit %>%
  select(date_admitted, .mean) %>%
  left_join(test_weekly_df, by = "date_admitted")

forecast_perf(
  exp(precip_pre_perf$.mean),
  exp(precip_pre_perf$n), "SARIMA(1,0,1)(1,1,0) + precip"
)

rh_pre_perf <- sarimax_rh_chunks_fit %>%
  select(date_admitted, .mean) %>%
  left_join(test_weekly_df, by = "date_admitted")

forecast_perf(
  exp(rh_pre_perf$.mean),
  exp(rh_pre_perf$n), "SARIMA(1,0,1)(1,1,0) + rh"
)
