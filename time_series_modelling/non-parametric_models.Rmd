...
```{r}
v_pois_test <- function(x) {
  ci <- sapply(x, \(x) poisson.test(x)$conf.int)
  list(lower = ci[1, ], upper = ci[2, ])
}

mean_fit <- forecast_chunks(train_weekly_df, test_weekly_df, seq(0, 49, 3), MEAN, n)
mean_fit %<>% mutate(
  uppers = v_pois_test(round(.mean))$upper,
  lowers = v_pois_test(round(.mean))$lower
)
...
chunks_forecast_perf(pre_perf_df, "MEAN")
```
...
chunks_forecast_perf(pre_perf_df, "NAIVE")
```
...
chunks_forecast_perf(pre_perf_df, "Drift")
```
