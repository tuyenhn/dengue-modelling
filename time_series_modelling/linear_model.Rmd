...
```{r}
p_base <- ggplot(incidence_df, aes(x = date_admitted)) +
...
```{r}
skedastic::white(lm(n ~ date_admitted, data = incidence_df))
```
...
boxcox_white_df <- map(boxcox_mle$x, \(lambda) {
  tibble(
    ...
    white_pvalue = skedastic::white(
      lm(unlist(transformed_x) ~ incidence_df$date_admitted)
    )$p.value
  )
}) %>%
  list_c()
...
prophet_weekly <- incidence_df_weekly %>%
  model(
    prophet = prophet(n ~ season(period = 52, order = 10, type = "additive"))
  )
...
accuracy(prophet_weekly_forecast, incidence_df_full_weekly)

incidence_df_weekly_2019 <- incidence_df_full_weekly %>%
  filter(year(date_admitted) == 2019)
...
```{r}
prophet_weekly_w_holiday <- incidence_df_weekly %>%
  model(
