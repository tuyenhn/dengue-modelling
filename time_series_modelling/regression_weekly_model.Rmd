...
precip_lag_df <- foreach(lag = 1:52) %dopar% {
  ...
  tibble(model = sprintf("Poisson %d-week precip lag", lag), RMSE = rmse, MAE = mae, lag = lag)
}
...
rh_lag_df <- foreach(lag = 1:52) %dopar% {
  ...
  tibble(model = sprintf("Poisson %d-week rh lag", lag), RMSE = rmse, MAE = mae, lag = lag)
}
...
forecast_perf(
  poi_1wl_pre_perf$preds,
  poi_1wl_pre_perf$n, "Poisson all covariables lagged"
)
```
...
```{r}
nb_1wl_pre_perf <- nb_covar_forecast_df %>%
  select(date, preds) %>%
  left_join(test_weekly_df, by = c("date" = "date_admitted"))

forecast_perf(
  nb_1wl_pre_perf$preds,
  nb_1wl_pre_perf$n, "NB 1-week lag"
)
```
