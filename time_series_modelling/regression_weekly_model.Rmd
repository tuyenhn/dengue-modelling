...
chunks_forecast_perf <- function(frcst_df, model_name, metrics = c("RMSE", "MAE", "CRPS")) {
  what <- c(
    RMSE = quo(sqrt(mean((n - preds)^2))),
    MAE = quo(mean(abs(n - preds))),
...
recur_miso <- function(
  ...
  pbapply::pblapply(start_indices, \(start_idx) {
    ...
    if (distr == "poisson") {
      forecast_df %<>% mutate(
        dist = dist_poisson(preds),
        stdev_h = sqrt(preds) * sqrt(actualweek - startweek + 1),
        lowers = (preds - 1.96 * stdev_h),
        uppers = (preds + 1.96 * stdev_h)
      )
    }
