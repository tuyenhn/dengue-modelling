...
p1 <- forecast_plot_default(
  ...
  "4 week forecast - Poisson regression 1-week lag", FALSE
)
p1
...
auto_lagged_df <- pblapply(1:20, \(lag) {
...
### Distributed lag linear

```{r}
dlag_df <- incidence_weekly_weather_df
for (lag in 1:20) {
  for (covar in colnames(incidence_weekly_weather_df)[-c(1, 2)]) {
    colname <- sprintf("%s_l%d", covar, lag)
    dlag_df %<>% mutate(!!colname := lag(!!sym(covar), lag))
  }
}
dlag_df
```
...
poi_dlm_forecast_df <- recur_miso(
  train_df, test_df,
  seq(1, 49, 3),
  model = list(past_obs = 1), xreg_names = covar_names,
  link = "log", distr = "poisson"
...
forecast_plot_default(
  poi_dlm_forecast_df, test_weekly_df,
  "4 week forecast - Poisson regression distributed lag"
)
```
